...
function createRouter() {
    ...
    router.post("/feedback", tokens_1.requireAuth, async (req, res) => {
    ...
    router.get("/setup/download", (req, res) => {
        ...
        const queryProjectToken = req.query.projectToken;
        let projectId = null;
        ...
        if (queryProjectToken) {
            const proj = db_1.db.prepare("SELECT id, token, require_verification, workspace_id FROM projects WHERE token = ?").get(queryProjectToken);
            if (proj) {
                projectId = proj.id;
                projectSubmitToken = proj.token;
                requireVerification = proj.require_verification === 1;
                const ws = db_1.db.prepare("SELECT telegram_bot_token, telegram_chat_id FROM workspaces WHERE id = ?").get(proj.workspace_id);
                if (ws) {
            ...
            const payload = (0, users_1.verifyToken)(jwt);
            if (payload) {
                const proj = db_1.db.prepare("SELECT id, token, require_verification FROM projects WHERE workspace_id = ? ORDER BY created_at ASC LIMIT 1").get(payload.workspaceId);
                if (proj) {
                    projectId = proj.id;
                    projectSubmitToken = proj.token;
                    requireVerification = proj.require_verification === 1;
                }
                const ws = db_1.db.prepare("SELECT telegram_bot_token, telegram_chat_id FROM workspaces WHERE id = ?").get(payload.workspaceId);
                if (ws) {
                    telegramBotToken = ws.telegram_bot_token;
                    telegramChatId = ws.telegram_chat_id;
                }
...
**Worker stopped responding and nothing in the log for a while:** the bat loop itself may have died. Check .agentinbox/watchdog.log - it logs every 10 minutes whether the bat loop was alive or had to be restarted. If you see repeated "restarting start.bat" entries, something is killing the worker repeatedly - investigate why (e.g. a process killing all node.exe processes, or insufficient permissions).`;
    ...
    router.get("/agent/workspace", requireWorkspaceToken, (req, res) => {
        ...
        const tgCfg = tasks_1.taskQueries.getTelegramConfig(ws.id);
        res.json({
            workspace_id: ws.id,
    ...
    router.get("/agent/tasks/pending", requireWorkspaceToken, (req, res) => {
        ...
        let projects = db_1.db.prepare("SELECT id, require_approval, require_verification FROM projects WHERE workspace_id = ?").all(ws.id);
    ...
    router.post("/agent/tasks/:id/status", requireWorkspaceToken, (req, res) => {
        try {
            ...
            if (!updated) {
                res.status(409).json({ error: "Task already claimed by another Claude instance — skip it" });
                return;
            }
            res.json(updated);
        }
    ...
    router.post("/agent/tasks/:id/complete", requireWorkspaceToken, (req, res) => {
        try {
            const { summary_technical, summary_plain, screenshot_base64: rawScreenshot, verification_url } = zod_1.z.object({
            ...
            const result = tasks_1.taskQueries.completeTask(req.params.id, summary_technical, summary_plain, undefined, screenshot_base64, verification_url);
            if (!result) {
                res.status(404).json({ error: "Task not found" });
                return;
            }
