...
async function _sendPhoto(botToken: string, chatId: string, screenshotBase64: string, caption: string, replyToMessageId?: number): Promise<void> {
  try {
    ...
    if (replyToMessageId) parts.push(field("reply_to_message_id", String(replyToMessageId)));
    parts.push(Buffer.from(
      `--${boundary}${CRLF}Content-Disposition: form-data; name="photo"; filename="screenshot.png"${CRLF}Content-Type: image/png${CRLF}${CRLF}`
    ));
    parts.push(buf);
...
async function _downloadTelegramFile(botToken: string, fileId: string): Promise<{ data: string; name: string; content: string } | null> {
  try {
    const res = await fetch(`${apiUrl(botToken)}/getFile?file_id=${fileId}`);
    if (!res.ok) return null;
    const info = await res.json() as { ok: boolean; result?: { file_path: string } };
  ...
  try {
    ...
    for (const update of data.result) {
      ...
      if (!replyToId) {
        ...
        if (msg.photo) {
          // Pick highest res photo ≤ 1280px wide — clear enough for Claude to read UI details
          const photos = msg.photo as { file_id: string; width: number }[];
          const sorted = [...photos].sort((a, b) => b.width - a.width);
          const best = sorted.find(p => p.width <= 1280) || sorted[sorted.length - 1];
          attachment = await _downloadTelegramFile(ws.botToken, best.file_id);
          if (attachment) attachment.name = "photo.jpg";
        } else if (msg.document) {
          attachment = await _downloadTelegramFile(ws.botToken, msg.document.file_id);
          if (attachment) attachment.name = msg.document.file_name || attachment.name;
        }
...
async function getLatestUpdateId(botToken: string): Promise<number> {
  try {
